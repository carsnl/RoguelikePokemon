...
/**
 * Bulbasaur is one of the available Pokemon in the game. It is an NPC that has actions and
 * behaviours.
 * <p>
 * Created by:
 *
...
public class Bulbasaur extends Pokemon implements TimePerception {
   ...
   */
  private final int DAY_HP_CHANGE = -5;
  /**
   * Amount of hp change during the night
   */
  private final int NIGHT_HP_CHANGE = 5;
  /**
   * Amount of hp change during cursed night
   */
  private final int CHANCE_CURSE = 4; // chance to halve health (50/100 = 1/2)
  /**
   * Amount of hp change during cursed night
   */
  private final int CURSE_HEALTH_FACTOR = 4; // set to quarter health (1/4) when cursed
  ...
  public Bulbasaur() {
    ...
    this.registerInstance(); // TimePerception object
  ...
  public ActionList allowableActions(Actor otherActor, String direction, GameMap map) {
    ...
    if (!otherActor.hasCapability(Element.GRASS) && !otherActor.hasCapability(Status.TRAINER)) {
  ...
  public boolean addSpecialItem(Actor currentActor, Actor opposingActor, GameMap map) {
    ...
    if (map.locationOf(currentActor).getGround().hasCapability(Element.GRASS)) {
      isValid = true;
    }
    return isValid;
  ...
  public void cursedNightEffect() {
    ...
    if (gen.chanceOutcome(CHANCE_CURSE)) {
      ...
      System.out.print(
          String.format("%s had its maximum health reduced by three quarters. ", this.name));
